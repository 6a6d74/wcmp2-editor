...
- **Interactive map** for drawing the geospatial extent of a dataset as a bounding box or polygon, powered by Leaflet. Bounding boxes use a click-to-place-first-corner, click-to-complete interaction with a live preview rectangle. Six input modes are available: draw on map, global bounding box, country bounding box, manual N/E/S/W coordinate entry, direct GeoJSON editing, and null (non-spatial). Note: the map currently supports drawing a single polygon or point; editing multi-polygons on the map and creating multi-point or multi-polygon geometries via the draw tools are not yet supported — use the JSON input mode as a workaround, and expect these to be addressed in a future iteration
