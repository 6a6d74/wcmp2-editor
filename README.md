...
- **Controlled vocabulary pickers** sourced live from the [WMO Codes Registry](https://codes.wmo.int) (Earth System Disciplines, resource types) and the [WCMP2 codelists repository](https://github.com/wmo-im/wcmp2-codelists) (contact roles, link types), with static fallbacks when offline. Contact roles are restricted to the five defined in the WCMP2 standard: `licensor`, `producer`, `processor`, `host`, and `publisher`. Link relations are drawn from the full [IANA link relations registry](https://www.iana.org/assignments/link-relations/) (118 entries, embedded at build time as the IANA endpoint does not support CORS) plus any WCMP2-specific additions fetched dynamically at runtime. Additional concept vocabularies — including GRIB2 Parameters, BUFR4 Elements, and [CF Standard Names](https://vocab.nerc.ac.uk/standard_name) — are offered as quick-select suggestions in the custom vocabulary picker; a **View vocabulary ↗** link opens the selected vocabulary in a new tab for browsing. When the BUFR4 Elements scheme is selected, a guidance note explains that elements are organised in sub-tables and shows how to reference a specific sub-table URL or include the sub-table in the Concept ID
...
- **Contacts with optional fields** — each contact card exposes organisation and individual name as core fields; email, phone, country, city, contact instructions, position, hours of service, and identifier are all opt-in via a checkbox. Only checked fields are written to the exported JSON. Importing a record automatically pre-ticks the checkboxes for any fields present in that record. **Identifier uniqueness** is enforced across all contacts — a duplicate identifier is flagged inline. **Email** addresses are validated live against standard format rules (no leading, trailing, or consecutive dots in the local part; no hyphens at the start or end of domain labels). **Telephone** numbers must include an international dialling code (`+` prefix) and contain only valid characters (digits, spaces, hyphens, parentheses, and dots). Each contact card also includes an optional **Links** sub-section, using the same link editor (relation, MIME type, URL, title) as the main Links section
- **Clone contacts** — duplicate any existing contact with a single click; useful when a dataset has multiple contacts who share most fields (organisation, address, roles). When the source contact has an identifier, the clone automatically receives a `-copy` suffix (or `-copy-2`, `-copy-3`, … if that name is already taken) to preserve uniqueness
- **URL validation and health check** — all URL input fields (link href, Status URL in Core Properties, and vocabulary scheme URLs in Themes) include format validation with inline error messages. `http://` and `https://` URLs additionally have a **Test** button that sends an HTTP HEAD request and reports the response status (OK, Redirect, Error, or connection failure)
- **MQTT channel field** — when a link URL uses the `mqtt://`, `mqtts://`, `ws://`, or `wss://` scheme, an additional **MQTT channel** field appears below the URL. The channel name is validated against MQTT specification rules: it must not start with `$`, must not contain wildcard characters (`+` or `#`) or null characters, and must not exceed 65,535 characters
- **MIME type selection** — the MIME type field in each link uses the same combobox as the Relation field: type to filter and autocomplete from a curated list of common types (including WMO-specific formats such as `application/grib`, `application/bufr`, `application/netcdf`, and `application/prs.coverage+json`), or enter any custom MIME type not in the list
- **Access control (security)** — each link in the main Links section has an optional **Access control (security)** toggle. When enabled, a JSON editor pre-populated with an `http/basic` scheme template is shown. The JSON is validated in real time: each named security scheme must have a `type` of `http`, `apiKey`, `openIdConnect`, or `oauth2`, conforming to the [OpenAPI security scheme object](https://spec.openapis.org/oas/v3.1.1.html#security-scheme-object-0)
- **Persistent identifiers (PIDs)** — the Optional Fields section supports associating a dataset with one or more persistent identifiers. Preset buttons select common schemes (DOI, ARK, Handle); selecting a preset populates an editable scheme URL field, which can also be typed in freely. DOI, ARK, and Handle values are each validated against their respective format rules, with inline error messages and examples. When at least one PID has been added but no `cite-as` link exists, the editor displays a recommendation to add a link with relation `cite-as` for the preferred citation URL
...
- **Import existing records** — load a record from a local `.json` file (drag-and-drop or browse) or fetch directly from a URL. The record is validated before importing; if validation issues are found the editor displays an amber warning listing each issue but still allows the import, so partially-complete records from a previous session can be resumed. On the first edit after importing, the editor asks whether to update the `updated` timestamp
...
| `properties.wmo:dataPolicy` | `core` (open) or `recommended` (conditional access) — required when `properties.type` is `dataset` |
...
| `properties.externalIds` | Persistent identifiers (DOI, ARK, Handle, or custom scheme) |
