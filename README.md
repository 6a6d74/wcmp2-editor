...
- **Interactive map** for drawing the geospatial extent of a dataset as a bounding box or polygon, powered by Leaflet. Bounding boxes use a click-to-place-first-corner, click-to-complete interaction with a live preview rectangle. Five input modes are available: draw on map, country bounding box, manual N/E/S/W coordinate entry, direct GeoJSON editing, and null (non-spatial)
